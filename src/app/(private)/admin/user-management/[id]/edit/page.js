    ...
    <div className="mb-12 dark:text-white">
      ...
      <form className="flex items-center gap-4 mt-5" onSubmit={updateRoles}>
        <label className="border border-gray-300 dark:border-gray-700 px-4 py-1 rounded-md text-sm">
          <input type="checkbox" className="mr-2" defaultChecked={roles.includes(ROLE_CUSTOMER)}  onChange={() => setUserRoles(ROLE_CUSTOMER)}/>
          CUSTOMER
        </label>
         <label className="border border-gray-300 dark:border-gray-700 px-4 py-1 rounded-md text-sm">
          <input type="checkbox" className="mr-2" defaultChecked={roles.includes(ROLE_MERCHANT)}  onChange={() => setUserRoles(ROLE_MERCHANT)}/>
          MERCHANT
        </label>
         <label className="border border-gray-300 dark:border-gray-700 px-4 py-1 rounded-md text-sm">
